...
export const ContributionsDetails = () => {
  ...
  const transformed = getProducts().map(({id, name, prixUnitaire}) => ({
    key: id,
    value: name + ': ' + prixUnitaire,
  }));
  ...
  const submitPayment = async () => {
    ...
    fetch('https://api.sandbox.paypal.com/v1/oauth2/token', {
      ...
      headers: {
        ...
        Authorization:
          'Basic QVQ2UzBxTFhqb2lxajFBdmVnV0FMbGFxUkRNT29UV3ZLU0RFRXM1Zzh5OUJwM0J3UDAwcktlLUlFRUkxWWZMalY4M2FLd2N3X29OZ0FFcmU6RUoxT3hJbjEwYV9QdThxNFdTdHlIekdkMktXeGdoVmJQZVc0VDh6ZzBjeW1CSE5EbUdFZEo2MDVEVjQ2MC16RFUwOGcwUWt3RTBOSTNmN1g=',
      },
        ...
        <View style={styles.section}>
          <View style={styles.row}>
            <View style={{flex: 1, flexDirection: 'row'}}>
              <DateEvent dateDebut={event === undefined ? '0' : event.dateDebut} flexSize={0.87} />
            </View>
        ...
        <View style={styles.section}>
          <View style={{flexDirection: 'row'}}>
            ...
            <View
              ...
              <TouchableOpacity
                onPress={() => getContributionOuiNon(contribution)}>
                ...
                {produits.map((product: Product, index: number) => {
                  return (
                    <View key={index}>
                      <TouchableOpacity
                        key={index}
                        onPress={() => deleteProduct(produits, index)}>
                        <View
                          style={[
                            styles.itemContainerForm,
                            {marginVertical: 5},
                          ]}>
                          <Text
                            style={{textAlign: 'center', marginVertical: 10}}>
                            {product.name} :{' '}
                            {product.prixUnitaire.toLocaleString()}{' '}
                            {product.devise}
                          </Text>
                        </View>
                      </TouchableOpacity>
          ...
          <View style={styles.section}>
            <View style={{flexDirection: 'row'}}>
              ...
              <View style={[{marginLeft: 90, alignItems: 'flex-end'}]}>
                <Button
                  mode="contained"
                  onPress={() => makeContributions(item)}>
                  {t('Global.Save')}
                </Button>
              </View>
            </View>
