...
type InvitationDetailsProp = StackNavigationProp<
...
const eventName = event==undefined ? '' : event.name;
  ...
  const anneeDebut = parseInt(
    event === undefined ? '' : event.dateDebut.substring(0, 4),
  );
  const moisDebut =
    parseInt(event === undefined ? '' : event.dateDebut.substring(4, 6)) - 1;
  const jourDebut = parseInt(
    event === undefined ? '' : event.dateDebut.substring(6, 8),
  );
  const heureDebut = parseInt(
    event === undefined ? '' : event.heureDebut.substring(0, 2),
  );
  const minutesDebut = parseInt(
    event === undefined ? '' : event.heureDebut.substring(2, 4),
  );
